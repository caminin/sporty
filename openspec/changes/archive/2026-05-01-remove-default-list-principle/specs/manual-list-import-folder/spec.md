...
The system SHALL import lists from this folder only through explicit user/admin action.
...
- **THEN** le JSON est validé avec la même logique de migration/validation que les autres imports
- **THEN** une nouvelle liste est créée (ID non réservé) et activée si nécessaire
- **THEN** le fichier source peut être gardé, déplacé ou supprimé selon politique produit explicite
