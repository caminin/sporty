...
### Requirement: Group exercise references resolve from global catalog
The system SHALL store exercise placements on a training as `exerciseRefs` entries with `refId`, `exerciseId`, and optional `value` override. Resolution SHALL load definitions from the **global** catalog. Session display label for grouping SHALL use catalog `muscleGroup`, not named session groups.
...
### Requirement: Session group reference containers
**Reason**: Trainings use flat `exerciseRefs`; UI groups by `muscleGroup`.
