...
Between any two consecutive work steps in a session (including two series of the same exercise), the system SHALL insert a rest step of duration equal to the training `globalRestTime`. No rest step SHALL follow the final work step of the session.

#### Scenario: Rest between series of same exercise
- **WHEN** a reference has `series` 3 and `globalRestTime` is 20 seconds
- **THEN** the session sequence is work → rest 20s → work → rest 20s → work (before the next exercise or session end)
...
- **THEN** a rest step of `globalRestTime` separates the last work step of A from the first work step of B
